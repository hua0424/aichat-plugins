...
export async function handleResetSession(args: string[]): Promise<void> {
	...
	try {
		...
		console.error(`Error: reset-session result unknown (DELIVERY_UNKNOWN); confirm using --request-id ${requestId}, do not reset with a new ID`);
		process.exit(1);
	}
	...
	console.error(`Error: reset-session failed: ${error}${res.status === 503 ? ` (DELIVERY_UNKNOWN; confirm using --request-id ${requestId}, do not reset with a new ID)` : ''}`);
	process.exit(1);
}
