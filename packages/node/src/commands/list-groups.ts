/**
 * REQ-010 S4 — `aichat list-groups` CLI.
 *
 * A THIN client (like list-friends): resolves the agent session key OUT-OF-BAND from the env, POSTs
 * to the node-local loopback capability socket, prints the JSON result. No identity args — the group
...
export async function handleListGroups(_args: string[]): Promise<void> {
	...
	console.error(`Error: list-groups failed: ${error}`);
	process.exit(1);
}
