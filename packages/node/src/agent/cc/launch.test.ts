...
describe('buildCcLaunchCommand', () => {
	...
	it('respects a claudeBin override', () => {
		const cmd = buildCcLaunchCommand({
			token: 'tok',
			workspaceDir: '/ws',
	...
	it('safely quotes paths containing spaces', () => {
		const cmd = buildCcLaunchCommand({
			token: 'tok',
			workspaceDir: '/home/My Files/ws',
			settingsPath: '/home/My Files/ws/settings.json',
		});
		expect(cmd).toContain("cd '/home/My Files/ws'");
		expect(cmd).toContain("--settings '/home/My Files/ws/settings.json'");
	});

	it('escapes a single quote in a path (closes/reopens the quote)', () => {
		const cmd = buildCcLaunchCommand({
			token: 'tok',
			workspaceDir: "/home/o'brien/ws",
			settingsPath: '/ws/settings.json',
		});
		// POSIX single-quote escaping: ' → '\''
		expect(cmd).toContain("cd '/home/o'\\''brien/ws'");
	});

	it('contains the token and both paths', () => {
		const cmd = buildCcLaunchCommand({
			token: 'my-token',
			workspaceDir: '/ws',
			settingsPath: '/ws/settings.json',
